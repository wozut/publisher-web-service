...
fun buildCsvFileContent(questionnaireFillings: List<QuestionnaireFillingForAnalysis>): String =
    questionnaireFillings
        .map { questionnaireFilling -> questionnaireFilling.toCsvLine() }
        .let { csvLines: List<String> ->
            buildString {
                appendLine(CSV_HEADER)
                csvLines.forEach { csvLine -> appendLine(csvLine) }
            }
        }

private fun List<MultipleChoiceAnswerForAnalysis>.toCsvLine(): String =
    joinToString(separator = ",") { answer: MultipleChoiceAnswerForAnalysis -> answer.answerOption.value.value }

private fun QuestionnaireFillingForAnalysis.toCsvLine(): String =
    this.answers
        .filterIsInstance<MultipleChoiceAnswerForAnalysis>()
        .filterNot(::isAnswerToQuestionOfInterest)
        .sortedBy { answer: MultipleChoiceAnswerForAnalysis -> answer.answerOption.multipleChoiceQuestion.order }
        .toCsvLine()
        .prependEmptyAnswersForQuestionsOfInterest()
        .appendEmptyAnswersForQuestionsAtTheEnd()
        .insertTeamNameValue()

private fun isAnswerToQuestionOfInterest(multipleChoiceAnswerForAnalysis: MultipleChoiceAnswerForAnalysis) =
    multipleChoiceAnswerForAnalysis.answerOption.multipleChoiceQuestion.label in QuestionLabels.questionOfInterestLabels

private fun String.prependEmptyAnswersForQuestionsOfInterest(): String = ",,,,".plus(this)

private fun String.appendEmptyAnswersForQuestionsAtTheEnd(): String = this.plus(",,,,")

private fun String.insertTeamNameValue(): String =
    split(",")
        .toMutableList()
        .apply { add(4, "Useless team name") }
        .toList()
        .joinToString(",")
